...
class DBManager:
        ...
        try:
            with self.connection:
                self.connection.execute('''
                    INSERT OR REPLACE INTO USER (IP, last_visit_date)
                    VALUES (?, ?)
                ''', (ip_address, current_date))
    ...
    def getCount(self):
        self.connect()
        with self.connection:
            cur = self.connection.execute('SELECT COUNT(DISTINCT(IP)) FROM USER')
            return cur.fetchone()[0]

    def getDailyVisitors(self):
        self.connect()
        with self.connection:
            cur = self.connection.execute('''
                SELECT last_visit_date, COUNT(DISTINCT IP)
                FROM USER
                GROUP BY last_visit_date
                ORDER BY last_visit_date DESC
            ''')
            return cur.fetchall()
    
    def getLastVisit(self, ip_address):
        self.connect()
        with self.connection:
            cur = self.connection.execute('SELECT last_visit_date FROM USER WHERE IP = ?', (ip_address,))
            result = cur.fetchone()
    ...
    def check_and_update_last_visit(self, ip_address):
        ...
        with self.connection:
            ...
            if result:
                last_visit_time = datetime.strptime(result[0], '%Y-%m-%d %H:%M:%S.%f')
                if (current_time - last_visit_time) >= timedelta(seconds=60):
                    cursor.execute('''
                        UPDATE USER SET last_visit_time = ? WHERE IP = ?
                    ''', (current_time, ip_address))
                    return True
            else:
                cursor.execute('''
                    INSERT OR REPLACE INTO USER (IP, last_visit_date, last_visit_time)
                    VALUES (?, ?, ?)
                ''', (ip_address, current_time.strftime('%Y-%m-%d'), current_time))
                return True
        return False
...
def load_contents() :
    #topic - chapter
    contents = {
        "파이썬 기초": ["자료형", "제어문", "고급", "모듈과 패키지", "파일입출력", "OOP"],
...
def pandas_dataset():
        ...
        with open('data/seaborn/titanic.csv', "rb") as template_file:
...
df.head()
        ...
        import pandas as pd
        df = pd.read_csv('data/seaborn/titanic.csv')
        st.write(df.head())
...
def main() :
    ...
    with st.sidebar:
        ...
        st.write(f"Total Visitors: {db.getCount()}")
        
        st.write("------- **Visitor Information** -------")
        st.write(f"Your IP: {current_ip}")
        last_visit = db.getLastVisit(current_ip)
        if last_visit:
            st.write(f"Your last visit: {last_visit}")
        else:
            st.write("This is your first visit.")
        
        st.write("Daily Visitors:")
        daily_visitors = db.getDailyVisitors()
        for date, count in daily_visitors:
            st.write(f"{date}: {count}")
        
        visit_updated = db.check_and_update_last_visit(current_ip)
    
    # ... (나머지 main 함수 코드)

        if visit_updated:
                st.success("Welcome back! Your last visit time has been updated.")
